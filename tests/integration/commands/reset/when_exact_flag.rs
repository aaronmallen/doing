...
fn it_uses_exact_matching() {
  ...
  doing
    .run(["reset", "--search", "Important Task", "--exact"])
    .assert()
    .success();
  ...
  assert!(
    ...
    "expected exact match to reset entry, got: {important_line}"
  );
}
...
fn it_uses_exact_matching_with_short_flag() {
  ...
  doing
    .run(["reset", "--search", "Important Task", "-x"])
    .assert()
    .success();
  ...
  assert!(
    !important_line.contains("2026-03-22 15:00"),
    "expected exact match with -x to reset entry, got: {important_line}"
  );
}
