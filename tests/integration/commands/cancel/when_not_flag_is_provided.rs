...
#[test]
fn it_cancels_entries_not_matching_tag_filter() {
  ...
  doing.run(["cancel", "--tag", "project1", "--not"]).assert().success();
  ...
  assert!(
    ...
    "expected Task B (not matching @project1) to be cancelled, got: {task_b_line}"
  );
  ...
  assert!(
    ...
    "expected Task A (matching @project1) not to be cancelled, got: {task_a_line}"
  );
}
