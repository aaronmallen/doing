...
#[test]
#[ignore = "our CLI does not support --after flag for import (see #186)"]
fn it_imports_entries_after_time() {
  ...
  doing
    .run(["import", "--after", "2024-01-15", source_path.to_str().unwrap()])
    .assert()
    .success();
  ...
  assert!(
    ...
    "expected 'After cutoff entry' in doing file, got: {contents}"
  );
  assert!(
    !contents.contains("Before cutoff entry"),
    "expected 'Before cutoff entry' to be excluded, got: {contents}"
  );
}
