use std::fs;

use crate::support::helpers::DoingCmd;

#[test]
fn it_inverts_filter() {
  let doing = DoingCmd::new();

  fs::write(
    doing.doing_file_path(),
    "Currently:\n\t- 2026-03-22 15:00 | Task A @meeting\n\t- 2026-03-22 10:00 | Task B\n",
  )
  .expect("failed to write doing file");

  doing.run(["reset", "--tag", "meeting", "--not"]).assert().success();

  let contents = doing.read_doing_file();

  // Task B (not matching @meeting) should be reset
  let task_b_line = contents
    .lines()
    .find(|l| l.contains("Task B"))
    .expect("expected Task B");
  assert!(
    !task_b_line.contains("2026-03-22 10:00"),
    "expected Task B time to be reset (not matching filter), got: {task_b_line}"
  );

  // Task A (matching @meeting) should keep original time
  let task_a_line = contents
    .lines()
    .find(|l| l.contains("Task A"))
    .expect("expected Task A");
  assert!(
    task_a_line.contains("2026-03-22 15:00"),
    "expected Task A to keep original time, got: {task_a_line}"
  );
}
