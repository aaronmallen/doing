...
fn it_does_not_show_totals_without_flag() {
  ...
  doing
    .run(["now", "--back", "1h", "Recent no totals @project"])
    .assert()
    .success();
  doing.run(["done"]).assert().success();

  let with_totals = doing.run(["recent", "--totals"]).output().expect("failed to run");
  let without_totals = doing.run(["recent"]).output().expect("failed to run");
