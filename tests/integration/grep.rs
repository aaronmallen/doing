...
fn it_deletes_matching_entries() {
    ...
    doing
        .run(["now", "Working on project alpha"])
        .assert()
        .success();
    doing.run(["now", "Lunch break"]).assert().success();
    doing
        .run(["now", "Working on project beta"])
        .assert()
        .success();

    doing
        .run(["grep", "--delete", "project"])
        .assert()
        .success();

    let contents = doing.read_doing_file();
    assert!(
        contents.contains("Lunch break"),
        "non-matching entry should remain"
    );
    assert!(
        !contents.contains("project alpha"),
        "matching entry should be deleted"
    );
    assert!(
        !contents.contains("project beta"),
        "matching entry should be deleted"
    );
}
...
fn it_finds_entries_by_tag_name() {
    ...
    doing
        .run(["now", "Working on project @coding"])
        .assert()
        .success();
    doing.run(["now", "Lunch break"]).assert().success();
    doing
        .run(["now", "Meeting with team @planning"])
        .assert()
        .success();
    ...
    assert!(
        ...
        "entry with @coding tag should be included"
    );
}
...
fn it_finds_entries_matching_search_term() {
    ...
    assert_eq!(
        helpers::count_entries(&stdout),
        1,
        "grep should find 1 matching entry"
    );
    assert!(
        stdout.contains("project alpha"),
        "matching entry should be included"
    );
}
...
fn it_searches_with_exact_mode() {
    ...
    doing
        .run(["now", "Something else entirely"])
        .assert()
        .success();
...
fn it_searches_with_fuzzy_mode() {
    ...
    assert_eq!(
        helpers::count_entries(&stdout),
        1,
        "fuzzy search should match 1 entry"
    );
    assert!(
...
fn it_searches_with_pattern_mode_quoted_phrase() {
    ...
    doing
        ...
        .success();
    doing
        .run(["now", "Project alpha is great"])
        .assert()
        .success();
