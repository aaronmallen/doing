...
fn it_filters_entries_after_time() {
  ...
  // Compute safe back time that stays within today to avoid midnight timezone flakes.
  let now = chrono::Local::now();
  let mins_today = (now.hour() * 60 + now.minute()) as u64;

  // Need at least 120 minutes into the day so the "morning" entry is
  // meaningfully before the --after cutoff while both remain today.
  if mins_today < 120 {
    eprintln!("skipping: only {mins_today}m since midnight — too close to midnight for reliable after-time filtering");
    return;
  }
  ...
  assert!(
    ...
    "expected morning entry to be excluded by --after filter, got: {stdout}"
  );
}
