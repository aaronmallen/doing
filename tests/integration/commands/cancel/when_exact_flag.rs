...
fn it_uses_exact_matching() {
  ...
  doing
    .run(["cancel", "--search", "Important Task", "--exact"])
    .assert()
    .success();
  ...
  assert!(
    ...
    "expected exact match to cancel entry, got: {important_line}"
  );
}
...
fn it_uses_exact_matching_with_short_flag() {
  ...
  doing
    .run(["cancel", "--search", "Important Task", "-x"])
    .assert()
    .success();
  ...
  assert!(
    important_line.contains("@done"),
    "expected exact match with -x to cancel entry, got: {important_line}"
  );
}
