...
fn it_shows_time_intervals() {
  ...
  doing
    .run(["done", "--back", &format!("{today} 09:00"), "Completed task"])
    .assert()
    .success();

  let output = doing.run(["on", &today, "--times"]).output().expect("failed to run");
...
fn it_shows_with_short_flag() {
  ...
  let long_output = doing.run(["on", &today, "--times"]).output().expect("failed to run");
  let short_output = doing.run(["on", &today, "-t"]).output().expect("failed to run");
