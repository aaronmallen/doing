...
#[test]
fn it_finishes_entries_matching_search() {
  let doing = DoingCmd::new();

  fs::write(
    doing.doing_file_path(),
    "Currently:\n\t- 2026-03-22 11:00 | Task A something\n\t- 2026-03-22 10:00 | Task B other\n\t- 2026-03-22 09:00 | Task C something\n",
  )
  .expect("failed to write doing file");

  doing.run(["finish", "--search", "Task B", "5"]).assert().success();
  ...
  assert!(
    ...
    "expected Task C to remain unfinished, got: {task_c}"
  );
}
