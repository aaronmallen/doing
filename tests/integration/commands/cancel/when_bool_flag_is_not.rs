...
#[test]
fn it_cancels_entries_without_specified_tag() {
  ...
  doing
    .run(["cancel", "--tag", "project1", "--bool", "NOT"])
    .assert()
    .success();
  ...
  assert!(
    ...
    "expected Task B (without @project1) to be cancelled, got: {task_b_line}"
  );
  ...
  assert!(
    ...
    "expected Task A (with @project1) not to be cancelled, got: {task_a_line}"
  );
}
