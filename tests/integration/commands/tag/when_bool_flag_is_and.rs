...
#[test]
fn it_tags_entries_matching_all_filter_tags() {
  ...
  doing
    .run(["tag", "--tag", "project1,tag2", "--bool", "AND", "newtag"])
    .assert()
    .success();
  ...
  assert!(
    ...
    "expected Task A (both tags) to get @newtag, got: {task_a_line}"
  );
  ...
  assert!(
    !task_c_line.contains("@newtag"),
    "expected Task C not to get @newtag, got: {task_c_line}"
  );
}
